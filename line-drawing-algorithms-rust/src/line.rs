...
pub fn linea_ecuacion(fb: &mut Framebuffer, x0: i32, y0: i32, x1: i32, y1: i32, color: Color) {
    let m = (y1 - y0) as f32 / (x1 - x0) as f32; // pendiente
    let b = y0 as f32 - m * x0 as f32; // intercepto

    for x in x0..=x1 {
        let y = m * x as f32 + b; // 1 mult + 1 suma por pixel
        fb.set_pixel(x, y.round() as i32, color); // redondeo al pixel mas cercano
    }
}
...
pub fn dda(fb: &mut Framebuffer, x0: i32, y0: i32, x1: i32, y1: i32, color: Color) {
    ...
    for _ in 0..pasos {
        ...
        // Avanzamos sumando el incremento a la posición.
        // Es solo una suma por paso: por eso DDA es más rápido que la ecuación.
        x += x_inc;
        y += y_inc;
    }
}

// Bresenham: dibuja la linea usando solo aritmetica entera, valido para los 8 octantes.
